...
function parseDateWithServer(dateStr, serverId) {
    ...
    const sign = offset >= 0 ? "+" : "-";
    const pad = (n) => String(Math.abs(n)).padStart(2,
        "0");
...
function findBannerConfigByTime(timestamp, categoryContext, serverId) {
    ...
    if (categoryContext) {
        if (categoryContext.includes("weap") || categoryContext.includes("constant")) {
            targetType = "weapon";
        }
        else if (categoryContext.includes("new")) {
            targetType = "new-player";
        }
        else if (categoryContext.includes("joint")) {
            targetType = "joint";
        }
        else if (categoryContext === "standard") {
            targetType = "standard";
        }
        else if (categoryContext === "special") {
            targetType = "special";
        }
    }
    ...
    const candidates = banners.filter(b => {
        ...
        if (targetType) {
            ...
            const isBannerWeapon = b.type === "weapon" || b.id?.includes("weap");
    ...
    if (candidates.length > 0) {
        candidates.sort((a, b) => {
            const aOrder = parseDateWithServer(getBannerDates(a, serverId).startStr, serverId).getTime();
            const bOrder = parseDateWithServer(getBannerDates(b, serverId).startStr, serverId).getTime();
...
export function getInternalBannerType(rawId) {
    ...
    if (id.includes("weapon") || id.includes("wepon") || id.includes("constant") || id.includes("scathe")) {
        return rawId;
    }
...
Object.values(characters).forEach(c => {
    if (c.name) {
        nameMap[c.name.toLowerCase()] = c.name;
    }

    if (c.id) {
        nameMap[c.id.toLowerCase()] = c.name;
    }
});
...
Object.values(weapons).forEach(w => {
    ...
    if (w.id) {
        nameMap[w.id.toLowerCase()] = w.name;
    }
});
...
export function parseGachaLog(list) {
    ...
    return list
        ...
        .map((item, i) => {
            ...
            if (item.gachaTs) {
                dateObj = new Date(Number(item.gachaTs));
            } else if (item.ts) {
                dateObj = new Date(Number(item.ts) * 1000);
            } else if (item.time) {
            ...
            if (timeCounters[tsStr] === undefined) {
...
export function mergePulls(oldList, newList) {
    ...
    for (const p of combined) {
        ...
        const d = p.time instanceof Date
            ? p.time
            : new Date(p.time);
        ...
        if (map.has(dedupKey)) {
            ...
            if (!existing.rawPoolId && p.rawPoolId) {
                existing.rawPoolId = p.rawPoolId;
            }

            if (existing.isNew === undefined && p.isNew !== undefined) {
                existing.isNew = p.isNew;
            }

            if (existing.isFree === undefined && p.isFree !== undefined) {
                existing.isFree = p.isFree;
            }

            if (existing.status === undefined && p.status !== undefined) {
                existing.status = p.status;
            }

            if (existing.isGuaranteed === undefined && p.isGuaranteed !== undefined) {
                existing.isGuaranteed = p.isGuaranteed;
            }
        } else {
            if (seqId > 0) {
...
export function calculateBannerStats(pulls, bannerId, accountServerId = null) {
    ...
    const isWeaponType = bannerId.includes("weap")
        || bannerId.includes("wepon")
    ...
    if (!currentViewBanner && (bannerId.includes("special") || isWeaponType || isJointType)) {
        ...
        candidates.sort((a, b) => {
            const aOrder = parseDateWithServer(getBannerDates(a, accountServerId).startStr, accountServerId).getTime();
            const bOrder = parseDateWithServer(getBannerDates(b, accountServerId).startStr, accountServerId).getTime();

            return bOrder - aOrder;
        });

        const lastPullTime = pulls.length > 0
            ? pulls[pulls.length - 1].time.getTime()
            : Date.now();
        ...
        if (activeBanner) {
            currentViewBanner = activeBanner;
        } else if (candidates.length > 0) {
            currentViewBanner = candidates[0];
        }
    }
    ...
    pulls.forEach((pull) => {
        ...
        if (rateUpCounters[uniqueBannerKey] === undefined) {
        ...
        if (typeof pull.isFree === "boolean") {
            ...
            isFreePull = ((bannerId.includes("special") && !isWeaponType) || isJointType)
                && (30 <= countInThisBanner && countInThisBanner < 40);
        }
        ...
        if (rateUpCounters[uniqueBannerKey] >= hardPityLimit - 1) {
        ...
        if (pull.rarity === 6) {
            ...
            if (!historicConfig) {
                historicConfig = findBannerConfigByTime(pull.time, pull.rawPoolId, accountServerId);
            }

            const featuredList = historicConfig?.featured6
                || currentViewBanner?.featured6
                || [];

            const isFeatured = featuredList.some(fid => {
                const c = characters[fid];

                if (c && normalize(c.name) === itemName) {
                    return true;
                }

                const w = weapons[fid];

                if (w && normalize(w.name) === itemName) {
                    return true;
                }
    ...
    if (bannerId.includes("standard")) {
        ...
        if (hasReceivedRateUp || currentBannerMileage >= 120) {
            mileage = {
                show: true,
                current: currentBannerMileage % 240,
                max: 240,
                label: "bonus_copy_6"
            };
        } else {
    ...
    return {
        ...
        winRate: {
            won: won5050,
            total: total5050,
            percent: total5050 ? ((won5050 / total5050) * 100).toFixed(1) : 0
        }
    };
}
...
export function findLCSMatches(existingPulls, newPulls) {
    ...
    for (let i = 1; i <= n; i++) {
        ...
        const oldWeight = getWeight(oldP.rarity);
        for (let j = 1; j <= m; j++) {
...
export function validateAccountConsistency(existingPulls, newPulls, isRecoveryEnabled = false) {
    ...
    const overlaps = existingPulls.filter(p => {
        if (!p || !p.time || typeof p.time.getTime !== "function" || Number.isNaN(p.time.getTime())) {
            return false;
        }

        const t = p.time.getTime();

        return minNewTime <= t && t <= maxNewTime;
    });

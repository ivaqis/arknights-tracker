<script>
  ...
  import Icon from '$lib/components/Icons.svelte'; // Убедись, что путь верный
  import { fade, slide } from 'svelte/transition';

  export let options = []; // Array of { value: string, label: string }
  export let value = "";
  ...
  // --- ПАТТЕРНЫ (Взяты из твоей кнопки) ---
  const topoPattern = `
...
<div class="relative {className}" use:clickOutside>
  <button
    ...
    class="
      ...
      {variant === 'black' 
        ? 'bg-[#21272C] border-[#21272C] text-white hover:bg-[#2A3036]' 
        : 'bg-white border-gray-200 text-[#21272C] hover:border-gray-300'}
    "
  >
    <div class="absolute inset-0 z-0 pointer-events-none overflow-hidden {variant === 'black' ? 'text-white' : 'text-black opacity-30'}">
       {@html topoPattern}
    </div>

    <span class="relative z-10 font-sdk text-lg truncate pr-4">
      {selectedLabel}
    </span>
    ...
    <div class="absolute top-0 right-0 w-2 h-2 border-t border-r {variant === 'black' ? 'border-white/30' : 'border-black/10'}"></div>
    <div class="absolute bottom-0 left-0 w-2 h-2 border-b border-l {variant === 'black' ? 'border-white/30' : 'border-black/10'}"></div>
  </button>
  ...
  {#if isOpen}
    <div 
      ...
      class="
        ...
        {variant === 'black' ? 'bg-[#2A3036] border-[#3A4046] text-white' : 'bg-white border-gray-100 text-[#21272C]'}
      "
    >
          ...
          <li>
            <button
              ...
              class="
                ...
                {value === option.value 
                   ? (variant === 'black' ? 'bg-[#FFE145] text-[#21272C]' : 'bg-gray-100') 
                   : (variant === 'black' ? 'hover:bg-[#3A4046]' : 'hover:bg-gray-50')}
              "
            >
              {option.label}
            </button>

<script>
    ...
    export let onZoomWeaponImage = undefined;

    function openImageZoom(variant) {
        if (onZoomWeaponImage) {
            onZoomWeaponImage(variant);
        } else {
            selectedImageVariant = variant;
        }
    }
    ...
    $: if (browser && ($page.url.searchParams || id)) {
        const paramId = id;

<script>
    ...
    function getResColorClass(val) {
        ...
        if (percent === 100) return "text-[#21272C] dark:text-[#FDFDFD]";
        if (percent <= 40) return "text-[#800020] dark:text-[#ff6b8b]";
        if (percent < 70) return "text-orange-500 dark:text-orange-400";
...
<div class="{showBackButton ? 'min-h-screen md:px-8 md:py-3' : 'md:px-1 md:py-1 pb-10'} font-sans transition-colors">
    ...
    <div class="w-full max-w-[1500px] mx-auto grid grid-cols-1 {showBackButton ? 'xl:grid-cols-12' : 'grid-cols-1'} gap-8 items-start">
        
        <div class="col-span-1 {showBackButton ? 'xl:col-span-7' : 'w-full'} flex flex-col gap-6">
            
            <div class="bg-white dark:bg-[#2b2b2b] rounded-3xl flex flex-col overflow-hidden border border-gray-200 dark:border-[#444] transition-colors {showBackButton ? 'min-w-[400px]' : ''}">
        ...
        <div class="col-span-1 {showBackButton ? 'xl:col-span-5' : 'w-full'} flex flex-col gap-6">

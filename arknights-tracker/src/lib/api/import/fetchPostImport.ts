...
export async function fetchPostImport(token: string, serverIds: string[], privateId: string | null, recovery: boolean = false): Promise<AsyncGenerator<PostImportGenericResponse, void, unknown>> {
    ...
    const response = await fetch(url, {
        ...
        body: JSON.stringify({ privateId: privateId, recovery: recovery })
    });
